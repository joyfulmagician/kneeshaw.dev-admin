...
interface JobServiceEditDialogProps {
  id: string;
  onUpdated: () => void;
}

export default function JobServiceEditDialog({
  id,
  onUpdated
}: JobServiceEditDialogProps) {
  ...
  const handleSaveClick = async () => {
    try {
      await updateJobService(id, values);
      setOpen(false);
      onUpdated();
      toast("Job service updated successfully.", { type: "success" });
    } catch (err: any) {
  ...
  const initialize = async () => {
    try {
      const res = await getJobService(id);
      setValues({
